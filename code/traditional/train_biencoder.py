...
# os.environ["TRANSFORMERS_CACHE"] = "/tmp2/b10705005/cache/hf/transformers"
...
class BiEncoderTrainer(object):
    def __init__(self, 
        ...
        # Datasets.
        documents_df = pd.read_csv(documents_filepath, dtype={"id": str, "article": str})
        train_queries_df, val_queries_df = self.split_train_val(queries_filepath, train_frac=0.9)
    ...
    def split_train_val(self, queries_filepath: str, train_frac: float):
        ...
        df = pd.read_csv(queries_filepath, dtype={"id": int, "question": str, "article_ids": str})
    ...
    def fit(self):
        ...
        for epoch in tqdm(range(self.epochs),  desc="Epoch"):
            ...
            scores = self.evaluator(model=self.model, device=self.device, batch_size=self.batch_size*3, epoch=epoch, writer=self.writer)
            if scores["macro_f1"] > best_f1:
                best_f1 = scores["macro_f1"]
                best_epoch = epoch
                self.model.save(join(self.output_path, "best"))
            # Save the model every 10 epoch
            if (epoch+1)%10 == 0:
                self.model.save(join(self.output_path, f"{epoch+1}"))

            # Report average loss and number of correct predictions.
            with open(join(self.output_path, "log.txt"), "a") as f:
                f.write(f'Epoch {epoch}: Train loss {(train_loss/num_batches):>8f} - Accuracy {(train_correct/num_samples*100):>0.1f}%')
        ...
        with open(join(self.output_path, "best", "log.txt"), "w") as f:
            f.write(f'best epoch: {best_epoch} f1 score: {best_f1}\n')
...
if __name__ == '__main__':
    ...
    model = BiEncoder(is_siamese=True,
                      q_model_name_or_path='hfl/chinese-roberta-wwm-ext',
                      truncation=True,
                      max_input_len=1000,
                      chunk_size=200,
                      window_size=20,
    ...
    trainer = BiEncoderTrainer(model=model, 
                               ...
                               queries_filepath=abspath(join(__file__, "data/train_data.csv")),  #data/train_data_gpt.csv
                               documents_filepath=abspath(join(__file__, "data/law.csv")), #data/law_gpt.csv
                               batch_size=16, 
                               epochs=100,
                               warmup_steps=500,
                               log_steps=10, 
